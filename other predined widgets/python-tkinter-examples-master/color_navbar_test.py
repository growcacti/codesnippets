...
def get_images(paths):
    ...
    return [
        PhotoImage(file=path) for path in paths
    ]  ##[ImageTk.PhotoImage(img) for img in pil_img]
...
class ColorNavBar(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        image_files = [
            os.path.join(base_icon, name.lower() + ".png") for name in section_names
        ]
        images = get_images(image_files)
        self.sections = [
            (image, section) for image, section in zip(images, section_names)
        ]
        self.frames = []
    ...
    def init_widgets(self):
        ...
        for index, section in enumerate(self.sections):
            ...
            print(section)
            self.list_img[index] = ttk.Label(
                frame, image=section[0], style="dark.TLabel"
            )
...
def init_styles():
    ...
    s.configure(
        "light.TLabel", background=light_purple, font=("Quicksand", 12, "roman")
    )
