...
class BatchRenamePlugin(Plugin):
    ...
    def addFolder(self, path=None):
        ...
        if path == None:
            path = filedialog.askdirectory(
                parent=self.main, initialdir=self.currentdir, title="Select folder"
            )
        if path:
            self.path = path
            # self.refresh()
    ...
    def execute(self):
        ...
        n = messagebox.askyesno("Rename", "Rename the files?", parent=self.master)
        if not n:
            return
...
def doRename(files=None, wildcard=None, pattern="", replacement="", rename=False):
    ...
    for pathname in files:
        ...
        if new_filename != basename:
            ...
            if rename == True:
                os.rename(
                    pathname, os.path.join(os.path.dirname(pathname), new_filename)
                )
    return newfiles
...
def main():
    ...
    parser = OptionParser()
    parser.add_option("-d", "--dir", dest="directory", help="Folder of raw files")
