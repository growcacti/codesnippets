...
class App:
    def __init__(self):
        ...
        Label(
            ...
            bg=self.current_bg,
            fg="white",
        ).grid(row=0, column=2, columnspan=2, pady=epad)
        Label(
            self.main, text="Timer", font=titlefont(24), bg=self.current_bg, fg="white"
        ).grid(row=0, column=4, pady=epad)
        Label(
            self.main, text="Status", font=titlefont(24), bg=self.current_bg, fg="white"
        ).grid(row=0, column=5, pady=epad)
        for i in range(5):
            Label(
                self.main,
                text="FlyPAD " + str(i + 1),
                font=bodyfont(24),
                bg=self.current_bg,
            ).grid(row=i + 1, column=0, pady=epad)
            ...
            Button(
                ...
                font=titlefont(12),
                fg=self.current_bg,
                command=self.timings[i].start,
            ).grid(row=i + 1, column=2, pady=epad, padx=bxpad)
            Button(
                self.main,
                text="■",
                font=titlefont(24),
                fg=self.current_bg,
                command=self.timings[i].stop,
            ).grid(row=i + 1, column=3, pady=epad, padx=bxpad)
        ...
        self.main.configure(
            bg=self.current_bg, borderwidth=10, font=bodyfont(18), fg="white"
        )
