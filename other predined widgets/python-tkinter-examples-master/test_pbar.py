...
class App(object):
    ...
    def make_widgets(self, parent):
        ...
        self.progressbar = ttk.Progressbar(
            parent, length=200, maximum=10, mode="indeterminate"
        )
        self.progressbar.pack()
        self.button_text = tk.StringVar()
        self.button_text.set(self.command)
        button = ttk.Button(parent, textvariable=self.button_text, command=self.toggle)
        button.pack()
        button.focus()
    ...
    def stop_process(self):
        if self.process is not None and self.process.poll() is None:
            ...
            self.after(10000, kill_process, self.process)
        self.process = None
