...
class Navbar(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.sections = [
            [
                ImageTk.PhotoImage(
                    Image.open(os.path.join(base_icon, "experiments_tiny.png"))
                ),
                "Experiments",
            ],
            [
                ImageTk.PhotoImage(Image.open(os.path.join(base_icon, "videos.png"))),
                "Videos",
            ],
            [
                ImageTk.PhotoImage(Image.open(os.path.join(base_icon, "sessions.png"))),
                "Sessions",
            ],
            [
                ImageTk.PhotoImage(Image.open(os.path.join(base_icon, "datasets.png"))),
                "Datasets",
            ],
        ]
        self.frames = []
    ...
    def click(self, event):
        ...
        if index != self.selection:
            self.change_style(
                self.frames[self.selection], "dark.TFrame", "light.TLabel"
            )
...
def init_styles():
    ...
    s.configure(
        "TCheckbutton", background=light_grey, font=("Open Sans", 10, "regular")
    )

    s.configure(
        "large.TLabel", background=light_grey, font=("Open Sans", 14, "regular")
    )
    s.configure(
        "medium.TLabel", background=light_grey, font=("Open Sans", 12, "regular")
    )
    s.configure(
        "small.TLabel", background=light_grey, font=("Open Sans", 10, "regular")
    )
    ...
    s.configure(
        ...
        foreground=blue,
        cursor="hand2",
        font=("Open Sans", 10, "regular"),
    )

    s.configure("dark.TFrame", background=dark_purple)
    s.configure("light.TFrame", background=light_purple)
    s.configure(
        "dark.TLabel", background=dark_purple, font=("Open Sans", 12, "regular")
    )
    s.configure(
        "light.TLabel", background=light_purple, font=("Open Sans", 12, "regular")
    )
