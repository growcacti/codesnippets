...
class PngImageTk(object):
    ...
    def __init__(self, filename):
        ...
        self.pixeldata = list(
            self.pixels
        )  # pixeldata has each row of the image as an array
        self.x = 0
        self.y = 0
        self.image = PhotoImage(
            width=self.w, height=self.h
        )  # use photoimage as temporary oject to write to canvas
    ...
    def convert(self):
        ...
        pixelrows = tuple(
            tuple(x) for x in pixelrows
        )  # convert our list of lists into a tuple of tuples
