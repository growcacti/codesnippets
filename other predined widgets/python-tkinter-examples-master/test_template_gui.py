...
class DataBase(object):
    def __init__(self):
        self.data = {"experiments": [], "videos": [], "session": [], "dataset": []}
    ...
    def add_experiment(self, name, date, status, **kwargs):
        ...
        with open(
            "./data/experiments/{}_{}.yaml".format(entry["id"], entry["name"]),
            "w",
            encoding="utf8",
        ) as f:
            yaml.dump(
                entry, f, default_flow_style=False, allow_unicode=True, canonical=False
            )
        ...
        for cascade, command in struct.items():
            ...
            for label, func in command.items():
                if "&sep" in label:
                    ...
                    submenu.add_command(label=label, command=func)
...
class MainWindow(object):
    def __init__(self, master):
        ...
        self.exp_list = MultiList(
            exps_tab, ["ID", "Title", "Starting date", "Status"], height=10
        )
    ...
    def load_db(self):
        askload = messagebox.askquestion(
            "Load file",
            "Are you sure to load a database from file? All unsaved data will be lost.",
            icon="warning",
        )
        if askload == "yes":
            self.currfile = filedialog.askopenfilename(
                title="Load database file", initialdir=self.dir
            )
    ...
    def new_db(self):
        askload = messagebox.askquestion(
            "New file",
            "Are you sure to open a new database? All unsaved data will be lost.",
            icon="warning",
        )
        if askload == "yes":
            self.currfile = None
    ...
    def saveas_db(self):
        self.currfile = filedialog.asksaveasfilename(
            title="Save database file", initialdir=self.dir
        )
        self.save_db()
