...
class Notebook:
    ...
    def build_Menu(self):
        ...
        for i, k, l in [
            ...
            if i == "Delete":
                self.filemenu[2].add_command(
                    label=(f"{i} {k:>20}"), command=lambda: self.canvas.delete(ALL)
                )
            elif l == "s":
    ...
    def build_page_1(self):
        self.canvas = Canvas(
            self.tab[1], width=100, height=50, bg="#ffffff", cursor="heart"
        )
        self.canvas.pack(fill=BOTH, expand=True, padx=5, pady=5)
        self.canvas.bind("<B1-Motion>", self.bind_paint)
        ttk.Button(
            self.tab[1], text="clean", command=lambda: self.canvas.delete(ALL)
        ).pack(side=LEFT, anchor=CENTER, padx=90, pady=5, fill=X, expand=1)
        ttk.Button(self.tab[1], text="close", command=quit).pack(
            side=LEFT, padx=90, fill=X, anchor=CENTER, expand=1
        )
