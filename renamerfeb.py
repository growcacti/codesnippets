...
class FileRenamerApp:
    ...
    def apply_rename(self, output_folder=None):
        ...
        start_number = 1  # Starting number for renaming
        for file in os.listdir(path):
            original_path = os.path.join(path, file)
            new_name = self.generate_newfilename(file, start_number, use_hex=False, prefix=False)
            ...
            try:
                ...
                start_number += 1  # Increment the number for the next file
            except OSError as e:
    ...
    def generate_newfilename(self, original_filename):
