...
class simpleCalender:
    ...
    def __init__(self, master):
        ...
        self.spinbxyear = tk.Spinbox(
            master, from_=1900, to=datetime.datetime.now().year, textvariable=self.year
        )
        ...
        self.combobox = ttk.Combobox(
            master, textvariable=self.month
        )  # textvariable - variable tied to value selected in combobox
        self.combobox.pack()
        ...
        self.lastday = 31
        self.spinbxday = tk.Spinbox(
            master, from_=1, to=31, textvariable=self.dateofmonth
        )
        self.spinbxday.pack()
    ...
    ################ METHODS ####################################################
    def getyear(self):
        if int(self.year.get()) in range(
            int(self.spinbxyear["from"]), int(self.spinbxyear["to"] + 1)
        ):
