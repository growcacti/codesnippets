...
class GreetingApp:
    def __init__(self, master):  # constructor method
        ...
        self.greet_list = (
            "Hello, World! This is python GUI.",
        ...
        self.btn = ttk.Button(
            master, text="Greet Again", command=self.handle_text
        )  # create a button
        ...
        self.label.img = tk.PhotoImage(
            file=imgpath
        )  # read_image :: saving image within label_object to prevent its garbage collection
        ...
        self.label.config(
            justify=tk.CENTER
        )  # justify text in label to (CENTER, RIGHT or LEFT)
        self.label.config(
            foreground="blue", background="yellow"
        )  # insert font color (foreground) and background color
        self.label.config(
            font=("Times", 10, "bold")
