...
def on_app_launch(frame: tk.Frame, width: int = 100, height: int = 100):
    ...
    # ! Creating frames
    settings_frame = tk.Frame(frame, **settings_frame_params)
    canvas_frame = tk.Frame(frame, **canvas_frame_params)
    ...
    color_definition = tk.Entry(settings_frame, textvariable=color_var)
    color_definition.grid(row=0, column=0)
    validate_color_definition = tk.Button(
        settings_frame, text="->", command=change_color
    )
    ...
    # Dot size entry
    dot_size = tk.Entry(settings_frame, textvariable=dot_size_var)
    dot_size.grid(row=1, column=0)
    validate_dot_size = tk.Button(settings_frame, text="->", command=change_dot_size)
    validate_dot_size.grid(row=1, column=1, sticky="w")

    # Import image
    import_image_path_var = tk.StringVar()
    import_image_path = tk.Entry(settings_frame, textvariable=import_image_path_var)
    import_image_path.grid(row=0, column=2, columnspan=2)
    import_image_button = tk.Button(
        settings_frame,
        text=TRANSLATIONS["Import"],
        command=partial(import_image, import_image_path_var),
    )
    import_image_button.grid(row=1, column=3, sticky="w")

    # ! Creating drawable zone
    globals()["main_canvas"] = tk.Canvas(
        canvas_frame, highlightthickness=2, bg="white", **canvas_size
    )
    ...
    canvas_width = tk.Entry(settings_frame, textvariable=canvas_width_var, width=10)
    canvas_width.grid(row=0, column=4)
    canvas_height_var = tk.IntVar()
    canvas_height_var.set(canvas_size["height"])
    canvas_height = tk.Entry(settings_frame, textvariable=canvas_height_var, width=10)
    ...
    reset_canvas_button = tk.Button(
        settings_frame, text=TRANSLATIONS["Reset"], command=reset_canvas
    )
    reset_canvas_button.grid(row=0, column=6, rowspan=2)
    ...
    canvas_frame.place(x=0, y=settings_frame_params["height"])
...
def display(event):
    ...
    globals()["main_canvas"].create_arc(x1, y1, x2, y2, fill=color, outline=color)
...
def getter(widget, filename):
    ...
    path = (
        f"../../{software_api.REGISTRY['USERS_FOLDER']}/{software_api.current_user}"
    ...
    if not os.path.exists(path):
        try:
            os.mkdir(
                f"../../{software_api.REGISTRY['USERS_FOLDER']}/{software_api.current_user}/_images"
            )
        except:
            pass
        try:
            os.mkdir(
                f"../../{software_api.REGISTRY['USERS_FOLDER']}/{software_api.current_user}/_images/_{software_dir}"
            )
        except:
            pass
...
def import_image(path):
    ...
    globals()["main_canvas"].create_image(
        0, 0, image=globals()["imported_image_" + str(nbr_imported_images)], anchor="nw"
    )
...
def apply_canvas_size(width, height):
    width = width.get()
    height = height.get()
    globals()["main_canvas"].config(width=width, height=height)
