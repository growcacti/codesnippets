...
""" version 3 -----------------------------------
Dark mode
    window.py -> _text widget changed config
...
class Win:
    ...
    def input_filename(
        self, title="Enter a new name", sentence="Do not put the extension"
    ):
    ...
    def newfile2(self):
        ...
        if self.newfilename != "":
            with open(
                f"{self.folder}/{self.newfilename}", "w", encoding="utf-8"
            ) as file:
                pass
            self._lbx.insert(0, self.newfilename)
    ...
    def showlistitems(self):
        ...
        for file in list_of_items:
            if file.endswith(self.extension.lower()) or file.endswith(
    ...
    def show_img_win2(self):
        ...
        self.imgviewer = tk.Label(self.win2, image=img)
    ...
    def showcontent(self, evt):
        if self.extension != ".png":
            ...
            try:
                ...
                with open(
                    f"{self.folder}/{self.filename}", "r", encoding="utf-8"
                ) as file:
                    content = file.read()
                self._text.delete("0.0", tk.END)
...
if __name__ == "__main__":
    ...
    ver = "2.9.0.2"
    win = Win(__file__, ver, folder=FOLDER_FOR_FILES, extension=FILE_EXTENSION)
    win.root.mainloop()
