...
c.execute(
    "CREATE TABLE IF NOT EXISTS user (username TEXT NOT NULL PRIMARY KEY,password TEX NOT NULL);"
)
db.commit()
db.close()
...
class main:
    ...
    def widgets(self):
        ...
        Label(self.logf, text="Username: ", font=("", 20), pady=5, padx=5).grid(
            sticky=W
        )
        Entry(self.logf, textvariable=self.username, bd=5, font=("", 15)).grid(
            row=0, column=1
        )
        Label(self.logf, text="Password: ", font=("", 20), pady=5, padx=5).grid(
            sticky=W
        )
        Entry(
