...
from tkinter import Listbox, Entry, Button, Label, Radiobutton, StringVar, Spinbox, Checkbutton, IntVar
...
class FileRenamerApp:
    def __init__(self, root):
        ...
        self.list_button = Button(root, text="List Files", command=self.browse_directory)
        ...
        # Radiobuttons for renaming options
        self.extension_var = StringVar()
        self.extension_var.set(".txt")  # Default extension
        self.radiobutton_label = Label(root, text="Select New Extension:")
        self.radiobutton_label.grid(row=3, column=0, columnspan=4)
        self.radio1 = Radiobutton(root, text=".txt", variable=self.extension_var, value=".txt")
        self.radio2 = Radiobutton(root, text=".jpg", variable=self.extension_var, value=".jpg")
        ...
        self.characters_label.grid(row=5, column=0, columnspan=2)
        self.characters_spinbox = Spinbox(root, from_=0, to=100, width=5)
        self.characters_spinbox.grid(row=5, column=2, columnspan=2)

        # Entry widget for adding prefix/suffix
        self.prefix_suffix_label = Label(root, text="Prefix/Suffix:")
        self.prefix_suffix_label.grid(row=6, column=0, columnspan=2)
        self.prefix_suffix_entry = Entry(root, width=40)
        self.prefix_suffix_entry.grid(row=6, column=2, columnspan=2)
        ...
        self.add_option_label.grid(row=7, column=0, columnspan=4)
        ...
        self.add_prefix_radio.grid(row=8, column=0)
        self.add_suffix_radio.grid(row=8, column=1)
        ...
        self.enumerate_checkbox.grid(row=9, column=0, columnspan=4)

        # Button to rename files
        self.rename_button = Button(root, text="Rename Files", command=self.rename_files)
        self.rename_button.grid(row=10, column=0, columnspan=4)
