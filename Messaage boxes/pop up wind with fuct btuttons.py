...
class TopLevelApp:
    ...
    def getPop(self):
        ...
        ttk.Button(
            self.window, text="Normalize Master window", command=self.getMaster
        ).pack()
        self.window.config(padx=100, pady=50)
        ttk.Button(self.window, text="Create pop-up", command=self.getPopup).pack()
        ttk.Button(
            self.window, text="Iconify(Minimize)", command=self.iconifywindow
        ).pack()
    ...
    def getPopup(self):
        ...
        ttk.Button(
            self.window2, text="Resize Window - TRUE", command=self.makeResizeTrue
        ).pack()
        ttk.Button(
            self.window2, text="Resize Window - FALSE", command=self.makeResizeFalse
        ).pack()
